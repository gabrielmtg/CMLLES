    ...
    with open(filepath, 'w') as f:
        ...
        for layer_idx in [0, 2, 4]:
            ...
            for i in range(w.shape[0]):
                # O Genann exige o Bias PRIMEIRO e com o sinal INVERTIDO
                f.write(f"{-b[i]:.8f}\n") 
...
def export_cmsis_weights(model, filepath):
    ...
    with open(filepath, 'w') as f:
        ...
        for layer_idx in [0, 2, 4]:
            ...
            # Matriz de pesos (já está no formato out_features x in_features)
...
def train_and_export(folders, window_size=10):
    ...
    cmsis_weights_path = os.path.join(model_dir, "iaes_cmsis_weights.txt")
    ...
    # Substituindo ReLU por Sigmoid para parear com o backend C puro
    ...
    # 1. Exporta os pesos para C PRIMEIRO (Garante que o CMSIS e Genann funcionem)
    export_genann_weights(model, genann_weights_path)
    export_cmsis_weights(model, cmsis_weights_path)

    # 2. Tenta exportar o ONNX com opset mais antigo, ignorando falhas do PyTorch Nightly
    ...
    try:
        ...
        print("\n[Aviso] Falha na exportação ONNX devido a um bug interno do PyTorch 2.8+.")
        print("Os arquivos de pesos para Genann e CMSIS-DSP foram gerados com sucesso e você pode prosseguir com a compilação em C.")
