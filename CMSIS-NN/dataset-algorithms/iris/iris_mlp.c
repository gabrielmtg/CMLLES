...
static void run_inference(const int8_t *input, int8_t *output)
{
    ...
    /* multiplier/shift pré-computados em train_iris.py a partir de
     * input_scale * FC1_WEIGHT_SCALE / HIDDEN_SCALE, no formato Q31
     * esperado por arm_nn_requantize(). O fator fixo "/8.0" usado
     * antes não correspondia à convenção do CMSIS-NN e fazia a
     * requantização colapsar tudo para 0. */
    fc1_quant.multiplier = FC1_MULTIPLIER;
    fc1_quant.shift = FC1_SHIFT;

    /* filter_dims formato [N, C]: N = profundidade de acumulação (INPUT_SIZE),
     * C = profundidade de saída (HIDDEN_SIZE). O código original usava
     * {1,1,INPUT_SIZE,HIDDEN_SIZE}, o que fazia filter_dims->n = 1 em vez
     * de INPUT_SIZE, e arm_nn_vec_mat_mult_t_s8 recebia rhs_cols=1 (em vez
     * de 4), processando só 1 coluna do produto vetor-matriz. */
    cmsis_nn_dims fc1_input_dims  = {1, 1, 1, INPUT_SIZE};
    cmsis_nn_dims fc1_filter_dims = {INPUT_SIZE, 1, 1, HIDDEN_SIZE};
    ...
    /* multiplier/shift pré-computados: HIDDEN_SCALE * FC2_WEIGHT_SCALE / OUTPUT_SCALE */
    fc2_quant.multiplier = FC2_MULTIPLIER;
    fc2_quant.shift = FC2_SHIFT;

    /* filter_dims formato [N, C]: N = HIDDEN_SIZE (accum depth), C = OUTPUT_SIZE */
    cmsis_nn_dims fc2_input_dims  = {1, 1, 1, HIDDEN_SIZE};
    cmsis_nn_dims fc2_filter_dims = {HIDDEN_SIZE, 1, 1, OUTPUT_SIZE};
