...
X_normal  = df_normal[feat_cols].values.astype(np.float32)

fann_path = os.path.join(MODEL_DIR, "nsl_ae_train.data")
with open(fann_path, "w") as f:
    f.write(f"{len(X_normal)} {input_dim} {input_dim}\n")
    for xi in X_normal:
        line = " ".join(f"{v:.6f}" for v in xi)
        f.write(line + "\n")
        f.write(line + "\n")

test_path = os.path.join(MODEL_DIR, "test_data.txt")
X_test = df_te[feat_cols].values.astype(np.float32)
...
print(f"Saved: {fann_path}, {test_path}")
