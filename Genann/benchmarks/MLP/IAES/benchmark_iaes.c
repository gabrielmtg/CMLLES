...
static void load_test_data(const char *path, TestData *td) {
    ...
    td->y = malloc(td->n_samples * sizeof(int));
...
int main(int argc, char *argv[]) {
    ...
    snprintf(model_path, sizeof(model_path), "model/iaes_%s_sigmoid_f64.genann", size);
    snprintf(label,      sizeof(label),      "iaes_%s_sigmoid_f64", size);
