...
static void load_test_data(const char *path) {
    ...
    test_X = malloc((size_t)n_test * N_INPUTS * sizeof(double));
    test_y = malloc((size_t)n_test * sizeof(int));
    for (int i = 0; i < n_test; i++) {
        for (int j = 0; j < N_INPUTS; j++) fscanf(f, "%lf", &test_X[i * N_INPUTS + j]);
        fscanf(f, "%d", &test_y[i]);
    }
    fclose(f);
}
...
int main(void) {
    ...
    for (int ci = 0; ci < N_CONFIGS; ci++) {
        ...
        snprintf(key,      sizeof(key),      "iaes_%s_sigmoid", cfg->size);
        snprintf(net_path, sizeof(net_path), "model/iaes_%s_sigmoid_f64.genann", cfg->size);
        ...
        int correct = 0;
        for (int i = 0; i < n_test; i++) {
            const double *out = genann_run(ann, &test_X[i * N_INPUTS]);
