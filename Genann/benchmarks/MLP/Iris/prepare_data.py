...
Y_train = np.eye(3, dtype=np.float64)[y_train]
with open(os.path.join(MODEL_DIR, "iris_genann_train.data"), "w") as f:
    f.write(f"{len(X_train)} 4 3\n")
    for xi, yi in zip(X_train, Y_train):
        f.write(" ".join(f"{v:.10f}" for v in xi) + "\n")
        f.write(" ".join(f"{v:.10f}" for v in yi) + "\n")
...
print(f"Saved: {MODEL_DIR}/iris_genann_train.data, {MODEL_DIR}/test_data.txt")
