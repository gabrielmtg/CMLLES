...
int main(int argc, char *argv[]) {
    ...
    if (hidden != 32 && hidden != 64 && hidden != 128) {
        fprintf(stderr, "Invalid hidden size.\n"); return 1;
    }

    char model_path[256], label[64], csv_path[256];
    snprintf(model_path, sizeof(model_path), "model/lstm_h%d_%s.tflite", hidden, prec);
    ...
    for (int i = 0; i < N_RUNS; i++) {
        ...
        TfLiteTensorCopyFromBuffer(in_tensor, td.X[idx], input_size * sizeof(float));
