...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <config> [f32|int8] [n_warmup]\n", argv[0]);
        return 1;
    }
    ...
    snprintf(param_path, sizeof(param_path), "model/cnn_%s_%s.ncnn.param", cfg, prec);
    snprintf(bin_path,   sizeof(bin_path),   "model/cnn_%s_%s.ncnn.bin",   cfg, prec);
    snprintf(label, sizeof(label), "cnn_%s_%s", cfg, prec);
