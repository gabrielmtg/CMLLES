...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <hidden> [f32|int8] [n_warmup]\n", argv[0]);
        return 1;
    }
    ...
    if (hidden != 32 && hidden != 64 && hidden != 128) {
        fprintf(stderr, "Invalid hidden size. Use 32, 64 or 128.\n");
        return 1;
    }

    char param_path[256], bin_path[256], label[64], csv_path[256];
    snprintf(param_path, sizeof(param_path), "model/lstm_h%d_%s.ncnn.param", hidden, prec);
    snprintf(bin_path,   sizeof(bin_path),   "model/lstm_h%d_%s.ncnn.bin",   hidden, prec);
    snprintf(label, sizeof(label), "lstm_h%d_%s", hidden, prec);
    ...
    int input_size = SEQ_LEN * INPUT_FEATS;
    ncnn_mat_t in_mat = ncnn_mat_create_2d(INPUT_FEATS, SEQ_LEN, NULL);

    for (int i = 0; i < n_warmup; i++) {
        float *ptr = (float *)ncnn_mat_get_data(in_mat);
        memcpy(ptr, td.X[i % td.n_samples], input_size * sizeof(float));
    ...
    for (int i = 0; i < N_RUNS; i++) {
        ...
        float *ptr = (float *)ncnn_mat_get_data(in_mat);
        memcpy(ptr, td.X[idx], input_size * sizeof(float));
        ncnn_extractor_t ex = ncnn_extractor_create(net);
        ncnn_extractor_input(ex, "in0", in_mat);
        ncnn_mat_t out_mat;
        ...
        float diff = out_data[0] - td.y[idx];
