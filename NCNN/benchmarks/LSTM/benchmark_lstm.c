...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <hidden> [n_warmup]\n", argv[0]);
        return 1;
    }
    ...
    snprintf(param_path, sizeof(param_path), "model/lstm_h%d_f32.ncnn.param", hidden);
    snprintf(bin_path,   sizeof(bin_path),   "model/lstm_h%d_f32.ncnn.bin",   hidden);
    snprintf(label, sizeof(label), "lstm_h%d_f32", hidden);
