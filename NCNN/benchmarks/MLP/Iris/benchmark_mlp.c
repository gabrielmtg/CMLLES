...
int main(int argc, char *argv[]) {
    ...
    snprintf(param_path, sizeof(param_path), "model/mlp_%s_%s_%s.ncnn.param", size, act, prec);
    snprintf(bin_path,   sizeof(bin_path),   "model/mlp_%s_%s_%s.ncnn.bin",   size, act, prec);
    snprintf(label, sizeof(label), "mlp_%s_%s_%s", size, act, prec);
    ...
    for (int i = 0; i < N_RUNS; i++) {
        ...
        for (int c = 1; c < n_classes; c++)
            if (out_data[c] > out_data[pred]) pred = c;
