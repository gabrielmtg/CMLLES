...
int main(int argc, char *argv[]) {
    ...
    snprintf(model_path, sizeof(model_path), "model/lstm_h%d_%s.onnx", hidden, prec);
    ...
    int64_t shape[] = {1, SEQ_LEN, INPUT_FEATS};
    const char *input_names[]  = {"input"};
    const char *output_names[] = {"output"};

    float *input_buf = malloc(input_size * sizeof(float));
    OrtValue *in_tensor = NULL;
    CHECK_ORT(g_ort->CreateTensorWithDataAsOrtValue(mem_info, input_buf,
        input_size * sizeof(float), shape, 3,
        ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &in_tensor));

    for (int i = 0; i < n_warmup; i++) {
        memcpy(input_buf, td.X[i % td.n_samples], input_size * sizeof(float));
        OrtValue *out_t = NULL;
    ...
    for (int i = 0; i < N_RUNS; i++) {
        ...
        memcpy(input_buf, td.X[idx], input_size * sizeof(float));
        OrtValue *out_t = NULL;
