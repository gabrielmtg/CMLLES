...
def bar_footprint_summary(df):
    ...
    fig, ax = plt.subplots(figsize=(9.5, 0.8 * len(frameworks) + 1.0))
    ...
    for j, platform in enumerate(PLATFORM_ORDER):
        ...
        for i, fw in enumerate(frameworks):
            ...
            ax.text(row.total_kb * 1.08, y_pos[i] + offset, label,
                    va="center", fontsize=6.5, color=HEADER_COLOR)

    ax.set_xscale("log")
    ax.set_yticks(y_pos)
    ax.set_yticklabels(frameworks)
    ax.set_xlabel("Static memory footprint, mean across algorithms (KB, log scale)")
    ax.set_title("Executable memory footprint by framework and platform")
    ax.set_xlim(right=max_total * 20)
