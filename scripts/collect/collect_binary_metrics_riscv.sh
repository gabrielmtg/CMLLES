...
mkdir -p "$RESULTS_DIR"
OUT="${RESULTS_DIR}/binary_metrics_riscv.csv"

if [ ! -d "$EXEC_DIR" ]; then
    echo "Error: ${EXEC_DIR} not found. Run cross_compile_tests_riscv.sh first."
    exit 1
fi
...
for f in "$EXEC_DIR"/*; do
    [ -f "$f" ] || continue
    name=$(basename "$f")
    read -r text data bss dec _ <<< "$(riscv64-linux-gnu-size "$f" | tail -1)"
...
for f in "$EXEC_DIR"/*; do
    ...
    name=$(basename "$f")
    echo "--- ${name} ---"
    riscv64-linux-gnu-objdump -h "$f" 2>/dev/null \
