...
LATENCIES_DIR="${OUTPUT_DIR}/latencies_riscv"
mkdir -p "$RESULTS_DIR"
OUTPUT="${RESULTS_DIR}/per_inference_perf_summary_riscv.csv"

if [ ! -d "$LATENCIES_DIR" ]; then
    echo "Erro: ${LATENCIES_DIR} não encontrado. Execute collect_from_sdcard_riscv.sh primeiro."
    exit 1
fi
